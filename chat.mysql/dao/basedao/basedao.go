...
func createFile(table string, columnMap *map[string][2]string, packageName string) string {
	...
	fileContent = fileContent + "\tbasedao.Table\n"
	...
	fileContent = fileContent + "func (t *" + tableName + ") Query(columns ...basedao.Column) ([]" + tableName + ",error) {\n"
	...
	fileContent = fileContent + "\t\tcolumns = []basedao.Column{ " + strings.Join(fs, ",") + "}\n"
	fileContent = fileContent + "\t}\n"
	...
	copy := `func copy` + tableName + `(channle chan int16, rows []interface{}, t *` + tableName + `, columns []basedao.Column) {
	...
	fileContent = fileContent + "func (t *" + tableName + ") QuerySingle(columns ...basedao.Column) (*" + tableName + ",error) {\n"
	fileContent = fileContent + "\tif columns == nil {\n"
	fileContent = fileContent + "\t\tcolumns = []basedao.Column{ " + strings.Join(fs, ",") + "}\n"
	fileContent = fileContent + "\t}\n"
	...
	fileContent = fileContent + "\t\tsetfield := \"Set\" + basedao.ToUpperFirstLetter(field)\n"
	fileContent = fileContent + "\t\treflect.ValueOf(rt).MethodByName(setfield).Call([]reflect.Value{reflect.ValueOf(basedao.GetValue(&core))})\n"
	fileContent = fileContent + "\t}\n"
	fileContent = fileContent + "\treturn rt,nil\n"
	fileContent = fileContent + "}\n\n"

	fileContent = fileContent + "func (t *" + tableName + ") Select(columns ...basedao.Column) (*" + tableName + ",error) {\n"
	fileContent = fileContent + "\tif columns == nil {\n"
	fileContent = fileContent + "\t\tcolumns = []basedao.Column{ " + strings.Join(fs, ",") + "}\n"
	fileContent = fileContent + "\t}\n"
	...
	fileContent = fileContent + "func (t *" + tableName + ") Selects(columns ...basedao.Column) ([]*" + tableName + ",error) {\n"
	fileContent = fileContent + "\tif columns == nil {\n"
	fileContent = fileContent + "\t\tcolumns = []basedao.Column{ " + strings.Join(fs, ",") + "}\n"
	fileContent = fileContent + "\t}\n"
	fileContent = fileContent + "\trows,err := t.Table.Selects(columns...)\n"
	...
	fileContent = fileContent + "func  cp" + tableName + "(buff []interface{}, t *" + tableName + ", columns []basedao.Column) {\n"
