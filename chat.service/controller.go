...
func controllerHandler(tt thrift.TTransport) {
	...
	tu := &User{Client: NewClient(tt), OverLimit: 3, Fw: FW.CONNECT, IdCardNo: utils.TimeMills(), Sendflag: make(chan string, 0), Sync: new(sync.Mutex)}
	TP.AddConnect(tu)
	defer func() {
	...
	//	}
	go Processor(tt, tu, gorutineclose, monitorChan)
	<-monitorChan
	//	errormsg := <-monitorChan
	//	logger.Error("errormsg:", errormsg)
}

func NewClient(tt thrift.TTransport) *IClient {
	transportFactory := thrift.NewTBufferedTransportFactory(1024)
	protocolFactory := thrift.NewTCompactProtocolFactory()
	useTransport := transportFactory.GetTransport(tt)
	return NewIClientFactory(useTransport, protocolFactory)
}

func Processor(client thrift.TTransport, tu *User, gorutineclose *bool, monitorChan chan string) error {
	defer func() {
		if err := recover(); err != nil {
			//			logger.Error(string(debug.Stack()))
	...
	//处理器
	handler := &Impl{Pub: pub, Client: client, Tu: tu}
	//解码器 + 读流写流
	processor := NewIProcessor(handler)
	for {
