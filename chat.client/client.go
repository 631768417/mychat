...
func (this *Connect) setIClient(client *IClient) {
	this.Client = client
}
...
type ConfirmBean struct {
	...
	confirmMBean   map[string]*MBean
	lock           *sync.RWMutex
}

var Confirm *ConfirmBean = &ConfirmBean{isClusterAck: false, confirmMessage: make(map[string]chan int, 0), confirmMBean: make(map[string]*MBean), lock: new(sync.RWMutex)}

func (this *ConfirmBean) Add(threadId string, mbean *MBean) {
	defer func() {
...
func NewCli(addr, username, pwd string, domain *string) (c *Cli) {
	c = &Cli{Sync: new(sync.Mutex), Flow: AUTH, Addr: addr, Name: username, Pwd: pwd, FlowChan: make(chan int, 0), Domain: domain}
	return
}

func (this *Cli) SendMBean(mbean *MBean) (err error) {
	defer func() {
		if er := recover(); er != nil {
	...
	if mbean != nil && mbean.ThreadId != "" {
		...
		if this.Addr != "" {
			err = this.Connect.Client.Message(mbean)
...
func (this *Cli) SendPBean(pbean *PBean) (err error) {
	defer func() {
		if er := recover(); er != nil {
			err = errors.New(fmt.Sprint(er))
	...
	if pbean != nil && pbean.ThreadId != "" {
		...
		if this.Addr != "" {
			err = this.Connect.Client.Presence(pbean)
			if err != nil {
	...
	for {
		...
		if this != nil && this.Flow == AUTH {
			...
			err := func() (err error) {
				...
				this.Sync.Lock()
				defer this.Sync.Unlock()
				err = this.Connect.Client.Ping(fmt.Sprint(currenteMillis()))
				logger.Debug("client ping<<<<<<<<<<<<<<<<<")
				return
			}()
			if err != nil {
...
func (this *Cli) Login() (err error) {
	...
	timclient := NewIClientFactory(useTransport, protocolFactory)
	if this.Connect != nil {
		this.Connect.Close()
	}
	this.Connect = &Connect{FlowConnect: CONNECT_START}
	this.Connect.setIClient(timclient)
	...
	err = timclient.Login(tid, this.Pwd)
	if err != nil {
